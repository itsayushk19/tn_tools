...
function SearchBar({ categorizedTools }) {
  ...
  useEffect(() => {
    ...
    Object.keys(categorizedTools).forEach((category) => {
      const tools = categorizedTools[category].filter((tool) =>
            ...
            <input
              ...
              onKeyDown={(e) => {
                if (e.key === "Tab" && Object.keys(filteredTools).length > 0) {
                  e.preventDefault();
                  const firstSuggestion = filteredTools[Object.keys(filteredTools)[0]][0].title;
                  setSearchQuery(firstSuggestion);
                }
              }}
            />
          </div>
          <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#000000" strokeWidth="1" strokeLinecap="round" strokeLinejoin="round">
                        ...
                        >
                          <div className='search_item'>
                            <div className='search_toolName' >
                              {tool.title.toLowerCase().includes(searchQuery.toLowerCase())
                                ? tool.title.split(new RegExp(`(${searchQuery})`, 'gi')).map((part, index) =>
